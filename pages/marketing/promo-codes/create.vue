    ...
    <template v-if="fetching">
      <Form id="add_promo_code_form" @submit="onSubmit" :validation-schema="schema" v-slot="{ errors }">
        <div class="flex flex-column gap-2 mb-1">
          ...
          <Field v-model="promo_code.name" id="name" name="name" :class="{ 'p-invalid': errors.name }" class="p-inputtext p-component" aria-describedby="promo-code-name-error" placeholder="Enter name"/>
          <small class="p-error" id="promo-code-name-error">{{ errors.name || '&nbsp;' }}</small>
        </div>
        <div class="flex flex-column gap-2 mb-1">
          <label for="description">Description</label>
          <Field type="textarea" v-model="promo_code.description" id="description" name="description" :class="{ 'p-invalid': errors.description }" class="p-inputtext p-component" aria-describedby="promo-code-description-error" placeholder="Enter description"/>
        ...
        <div class="flex flex-column gap-2 mb-1">
          ...
          <Field v-model="promo_code.discount_amount" id="discount_amount" name="discount_amount" :class="{ 'p-invalid': errors.discount_amount }" class="p-inputtext p-component" aria-describedby="promo-code-discount-amount-error" placeholder="Discount amount"/>
          <small class="p-error" id="promo-code-discount-amount-error">{{ errors.discount_amount || '&nbsp;' }}</small>
        </div>
        <div class="flex flex-column gap-2 mb-1">
          <label for="min_spend">Min Spend</label>
          <Field v-model="promo_code.min_spend" id="min_spend" name="min_spend" :class="{ 'p-invalid': errors.min_spend }" class="p-inputtext p-component" aria-describedby="promo-code-min-spend-error" placeholder="Min spend"/>
          <small class="p-error" id="promo-code-min-spend-error">{{ errors.min_spend || '&nbsp;' }}</small>
        </div>
        <div class="flex flex-column gap-2 mb-1">
          <label for="max_spend">Max Spend</label>
          <Field v-model="promo_code.max_spend" id="max_spend" name="max_spend" :class="{ 'p-invalid': errors.max_spend }" class="p-inputtext p-component" aria-describedby="promo-code-max-spend-error" placeholder="Min spend"/>
        ...
        <div class="flex flex-column gap-2 mb-1">
          ...
          <Field v-model="promo_code.per_coupon_limit" id="per_coupon_limit" name="per_coupon_limit" :class="{ 'p-invalid': errors.per_coupon_limit }" class="p-inputtext p-component" aria-describedby="promo-code-per-coupon-limit-error" placeholder="Enter per coupon limit"/>
        ...
        <div class="flex flex-column gap-2 mb-1">
          ...
          <small class="p-error" id="promo-code-remaining-coupon-error">{{ errors.remaining_coupon || '&nbsp;' }}</small>
        </div>
        <div class="flex flex-column gap-2 mb-1">
          ...
          <small class="p-error" id="promo-code-per-user-limit-error">{{ errors.per_user_limit || '&nbsp;' }}</small>
        </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const schema = Yup.object().shape({
      ...
      discount_amount: Yup.number().required().min(2).max(100).label("Discount amount"),
    ...
    return {
      schema, onSubmit, promo_code, loading, fetching, spinner, resetModal, categories, status_enums, discount_type_enums
    }
