...
export default defineComponent({
  ...
  setup(){
    ...
    const fetchChildProduct = async () => {
      const {data, error} = await useApiFetch("/api/child-products/" + route.params.id + '/edit', {
        method: "GET",
      });
      if (data.value) {
        const getChildProduct = JSON.parse(JSON.stringify(computed(() => data.value).value))
        child_product.value = getChildProduct.child_product;
      }
    };
    ...
    const onSubmit = async (values, actions: { setErrors: (arg0: any) => void }) => {
      ...
      const { data, error } = await useApiFetch("/api/child-products/update/" + route.params.id, {
        method: "POST",
