...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchFontCategories = async () => {
      const {data, error} = await useApiFetch("/api/size-categories/" + route.params.id + '/edit', {
        method: "GET",
      });
      if (data.value) {
        const getSIzeCategory = JSON.parse(JSON.stringify(computed(() => data.value).value))
        size_category.value = getSIzeCategory.size_category;
      }
    };
    ...
    const onSubmit = async (values: any, actions: { setErrors: (arg0: any) => void }) => {
      ...
      const { data, error } = await useApiFetch("/api/size-categories/" + route.params.id, {
        method: "PUT",
