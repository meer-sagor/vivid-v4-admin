<template>
  <div class="card">
    ...
    <template v-if="fetching">
      <Form id="add_clipart_form" :initial-values="clipart" @submit="onSubmit" :validation-schema="schema" v-slot="{ errors }">
...
export default defineComponent({
  ...
  setup: function () {
    ...
    const onSubmit = async (values: any, actions: { setErrors: (arg0: any) => void }) => {
      ...
      const {data, error} = await useApiFetch("/api/cliparts/store", {
        method: "POST",
        body: formData,
      });
      loading.value = false;
      ...
      if (data.value) {
        ...
        await router.push({ path: "/cliparts" });
        resetModal();
        resetForm();
      }
    };
    ...
    const resetModal = () => {
      const resetForm = document.getElementById("add_clipart_form") as HTMLFormElement;
      if (resetForm) {
        resetForm.reset();
      }
    };
