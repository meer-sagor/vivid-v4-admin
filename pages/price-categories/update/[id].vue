<template>
  <div class="card">
    <h5>Update Price Category</h5>
    <template v-if="fetching">
      <Form id="update_price_category_form" :initial-values="price_category" @submit="onSubmit" :validation-schema="schema" v-slot="{ errors }">
        <div class="flex flex-row gap-3">
          ...
          <div class="col-6 mb-0">
            <div class="flex flex-column gap-2 mb-0">
              ...
              <small class="p-error" id="font-size-error">{{errors.size_id || "&nbsp;"}}</small>
            </div>
          </div>
        </div>
...
export default defineComponent({
  ...
  setup: function () {
    ...
    const fetchPriceCategories = async () => {
      const {data, error} = await useApiFetch("/api/price-categories/" + route.params.id + '/edit', {
        method: "GET",
      });
      if (data.value) {
        const getPriceCategories = JSON.parse(JSON.stringify(computed(() => data.value).value))
        price_category.value = getPriceCategories.price_category;
      }
    };
    ...
    const onSubmit = async (values: any, actions: { setErrors: (arg0: any) => void }) => {
      ...
      const {data, error} = await useApiFetch("/api/price-categories/"+ route.params.id, {
        method: "PUT",
    ...
    const resetModal = () => {
      const resetForm = document.getElementById("update_price_category_form") as HTMLFormElement;
      if (resetForm) {
        resetForm.reset();
      }
    };
