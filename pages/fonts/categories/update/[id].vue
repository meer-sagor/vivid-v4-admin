        ...
        <div class="flex flex-row gap-3">
          <div class="col-6 mb-0">
            <div class="flex flex-column gap-2 mb-0">
              ...
              <ClientOnly>
                <QuillEditor
                    :content="font_category.description"
                    ref="editor"
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchFontCategories = async () => {
      const {data, error} = await useApiFetch("/api/font-categories/" + route.params.id + '/edit', {
        method: "GET",
      });
      if (data.value) {
        const getPromoCodes = JSON.parse(JSON.stringify(computed(() => data.value).value))
        font_category.value = getPromoCodes.font_category;
      }
    };
    ...
    const onSubmit = async (values: any, actions: { setErrors: (arg0: any) => void }) => {
      ...
      const { data, error } = await useApiFetch("/api/font-categories/" + route.params.id, {
        method: "PUT",
